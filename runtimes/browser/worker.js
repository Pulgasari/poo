...
async function compile(source, target) {
    ...
    if (!ast) {
        throw new Error("Parser-Fehler: AST konnte nicht erzeugt werden.");
    }

    // 3. Generieren (je nach Ziel)
    let generatedCode;
    if (target === 'browser') {
        generatedCode = generate(ast, 'browser');
    } else if (target === 'deno') {
        // Hier würdest du deinen Deno-Generator aufrufen
        generatedCode = generate(ast, 'deno'); 
    } else if (target === 'odin') {
        // Hier würdest du deinen Odin-Code-Generator aufrufen
        generatedCode = generate(ast, 'odin');
    } else {
